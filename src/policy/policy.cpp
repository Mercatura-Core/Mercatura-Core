...
bool AreInputsStandard(const CTransaction& tx, const CCoinsViewCache& mapInputs)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (whichType == TxoutType::NONSTANDARD ||
            ...
            // validation.
            return false;
        } else if (whichType == TxoutType::WITNESS_V2_MERCATURA_PQ) {
            // Mercatura PQ Authorization v1 is native witness-v2 only.
            // Consensus also requires this, but reject cheaply at policy.
...
bool IsWitnessStandard(const CTransaction& tx, const CCoinsViewCache& mapInputs)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        // Get the scriptPubKey corresponding to this input before deciding
        // whether an empty witness may be ignored. Mercatura PQ v1 requires
        // an exact non-empty two-item witness and must fail cheaply here.
        ...
        // Mercatura PQ Authorization v1 has one exact native witness form:
        //   witness[0] = raw 3309-byte ML-DSA-65 signature
        //   witness[1] = raw 1952-byte ML-DSA-65 public key
        //
        // P2SH wrapping, missing/extra items, and all size variants are
        // nonstandard. Cryptographic verification remains a consensus check.
        if (witnessversion == 2 &&
            witnessprogram.size() == WITNESS_V2_MERCATURA_PQ_SIZE) {
            if (p2sh) {
                return false;
            }
            ...
            if (stack[0].size() != MERCATURA_MLDSA65_SIGNATURE_SIZE) {
                return false;
            }
            if (stack[1].size() != MERCATURA_MLDSA65_PUBLIC_KEY_SIZE) {
                return false;
            }
        }

        // For other witness types, an empty witness cannot be bloated and
        // consensus validation will determine whether the spend is valid.
