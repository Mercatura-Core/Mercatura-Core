...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-bind=<addr>[:<port>][=onion]", strprintf("Bind to given address and always listen on it (default: 0.0.0.0). Use [host]:port notation for IPv6. Append =onion to tag any incoming connections to that address and port as incoming Tor connections (default: 127.0.0.1:%u=onion, testnet: 127.0.0.1:%u=onion, signet: 127.0.0.1:%u=onion, regtest: 127.0.0.1:%u=onion)", defaultChainParams->GetDefaultPort() + 100, testnetChainParams->GetDefaultPort() + 100, signetChainParams->GetDefaultPort() + 100, regtestChainParams->GetDefaultPort() + 100), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::CONNECTION);
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    const uint16_t default_bind_port_onion =
        args.IsArgSet("-port") ? default_bind_port + 1 : Params().GetDefaultPort() + 100;
