...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, TxValidationState& state,
                ...
                ValidationCache& validation_cache, const uint256& pq_genesis_hash)
                EXCLUSIVE_LOCKS_REQUIRED(cs_main, pool.cs)
{
    AssertLockHeld(cs_main);
    AssertLockHeld(pool.cs);
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckInputScripts(
            ...
            GetValidationCache(),
            nullptr,
            args.m_chainparams.GetConsensus().hashGenesisBlock)) {
...
bool CheckInputScripts(const CTransaction& tx, TxValidationState& state,
                       ...
                       std::optional<uint256> pq_genesis_hash)
{
    ...
    hasher.Write(reinterpret_cast<unsigned char*>(&flags), sizeof(flags));

    const unsigned char pq_context_present{
        static_cast<unsigned char>(pq_genesis_hash.has_value())
    };
    hasher.Write(&pq_context_present, 1);

    if (pq_genesis_hash.has_value()) {
        hasher.Write(pq_genesis_hash->begin(), 32);
    }
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        // Verify signature
        CScriptCheck check(
            txdata.m_spent_outputs[i],
            tx,
            validation_cache.m_signature_cache,
            i,
            flags,
            cacheSigStore,
            &txdata,
...
script_verify_flags GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    // Mercatura enables P2SH, witness, and Taproot semantics from chain
    // start. There are no inherited Bitcoin historical script exceptions.
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!CheckBlock(
            ...
            state,
            params.GetConsensus(),
            /*fCheckPOW=*/!fJustCheck,
            /*fCheckMerkleRoot=*/!fJustCheck,
            PoWCheckStatus::UNCHECKED,
            m_chainman.m_pow_hash_context.get())) {
        if (state.GetResult() == BlockValidationResult::BLOCK_MUTATED) {
...
{
    if (fCheckPOW) {
        // Free-standing validation callers may not provide a reusable context.
        // In that case create a local lazy context for this single check.
        PoWHashContext local_pow_context;
        PoWHashContext& context{
            pow_context ? *pow_context : local_pow_context};
    ...
    PoWHashContext* pow_context)
{
    ...
    // Check that the header is valid. The caller may explicitly report
    // that the cryptographic PoW for this exact header has already been
    // checked, avoiding a second memory-hard MercaHash invocation.
    const bool check_pow{
        fCheckPOW &&
        pow_check == PoWCheckStatus::UNCHECKED};
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // Check the required proof-of-work target for this height.
    const Consensus::Params& consensusParams = chainman.GetConsensus();
    ...
    // Testnet4 and regtest only: check the retained BIP94 timestamp
    // boundary to prevent timewarp attacks.
    if (consensusParams.enforce_BIP94) {
        // Check timestamp at each configured BIP94 timewarp boundary,
        // except the genesis block.
        if (nHeight % consensusParams.BIP94TimewarpInterval() == 0) {
            if (block.GetBlockTime() < pindexPrev->GetBlockTime() - MAX_TIMEWARP) {
                return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "time-timewarp-attack", "block's timestamp is too early at BIP94 timewarp boundary");
            }
        }
    }
    ...
    PoWCheckStatus pow_check)
{
    ...
    // AcceptBlockHeader() either checked the actual cryptographic PoW
    // above or the caller explicitly reported that it had already been
    // checked. Do not perform the memory-hard hash a second time here.
    //
    // fCheckPOW remains true so signet solution validation is preserved.
    if (!CheckBlock(
            block,
            state,
            params.GetConsensus(),
    ...
    const bool check_merkle_root)
{
    ...
    if (!CheckBlock(
            ...
            PoWCheckStatus::UNCHECKED,
            chainstate.m_chainman.m_pow_hash_context.get())) {
...
{
    // Construction remains cheap because PoWHashContext itself allocates its
    // 128 MiB scratchpad only on the first real MercaHash invocation.
    m_pow_hash_context = std::make_unique<PoWHashContext>();
}
