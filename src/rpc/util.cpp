...
CFeeRate ParseFeeRate(const UniValue& json)
{
    ...
    if (val >= COIN) throw JSONRPCError(RPC_INVALID_PARAMETER, "Fee rates larger than or equal to 1MCA/kvB are not accepted");
    return CFeeRate{val};
}
...
public:
    ...
    UniValue operator()(const WitnessV2MercaturaPQ& pq) const
    {
        UniValue obj(UniValue::VOBJ);
        obj.pushKV("isscript", true);
        obj.pushKV("iswitness", true);
