...
std::string GetTxnOutputType(TxoutType t)
{
    ...
    case TxoutType::WITNESS_V2_MERCATURA_PQ: return "witness_v2_mercatura_pq";
...
TxoutType Solver(const CScript& scriptPubKey, std::vector<std::vector<unsigned char>>& vSolutionsRet)
{
    ...
    if (scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram)) {
        ...
        if (witnessversion == 2) {
            if (witnessprogram.size() == WITNESS_V2_MERCATURA_PQ_SIZE) {
                vSolutionsRet.push_back(std::move(witnessprogram));
                return TxoutType::WITNESS_V2_MERCATURA_PQ;
            }

            // Witness version 2 is permanently assigned to Mercatura
            // PQ Authorization v1. No other v2 program size is defined.
