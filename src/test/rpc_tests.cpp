...
BOOST_AUTO_TEST_CASE(rpc_format_monetary_values)
{
    ...
    BOOST_CHECK_EQUAL(ValueFromAmount(std::numeric_limits<CAmount>::max() - 2).write(), "92233720368547758.05");
    BOOST_CHECK_EQUAL(ValueFromAmount(std::numeric_limits<CAmount>::max() - 3).write(), "92233720368547758.04");

    BOOST_CHECK_EQUAL(ValueFromAmount(std::numeric_limits<CAmount>::min() + 3).write(), "-92233720368547758.05");
    BOOST_CHECK_EQUAL(ValueFromAmount(std::numeric_limits<CAmount>::min() + 2).write(), "-92233720368547758.06");
    BOOST_CHECK_EQUAL(ValueFromAmount(std::numeric_limits<CAmount>::min() + 1).write(), "-92233720368547758.07");
    BOOST_CHECK_EQUAL(ValueFromAmount(std::numeric_limits<CAmount>::min()).write(), "-92233720368547758.08");
}
...
BOOST_AUTO_TEST_CASE(rpc_parse_monetary_values)
{
    ...
    BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("1")), COIN);
    BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("1.00")), COIN);
    BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("12345.67")), 1'234'567LL);

    // Scientific notation is valid when the result is representable
    // using Mercatura's two-decimal amount precision.
    BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("1e-2")), CENT);
    BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("0.1e-1")), CENT);
    BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("100e-2")), COIN);

    // Values below one Cent are not representable.
    BOOST_CHECK_THROW(AmountFromValue(ValueFromString("0.001")), UniValue);
    BOOST_CHECK_THROW(AmountFromValue(ValueFromString("1e-3")), UniValue);
    BOOST_CHECK_THROW(AmountFromValue(ValueFromString("0.009")), UniValue);

    // Trailing zeroes beyond the two significant decimals are acceptable.
