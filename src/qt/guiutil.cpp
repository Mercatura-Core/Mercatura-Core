...
{
    // return if URI is not valid or is no mercatura: URI
    if(!uri.isValid() || uri.scheme() != QString("mercatura"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("mercatura:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Mercatura Core.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Mercatura Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Mercatura Core (%s).lnk", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == ChainType::MAIN)
            optionFile << "Name=Mercatura Core\n";
        else
            optionFile << strprintf("Name=Mercatura Core (%s)\n", ChainTypeToString(chain));
