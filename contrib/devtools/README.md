...
gen-mercatura-conf.sh
===================

Generates a mercatura.conf file in `share/examples/` by parsing the output from `mercaturad --help`. This script is run during the
release process to include a mercatura.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-mercatura-conf.sh
```
