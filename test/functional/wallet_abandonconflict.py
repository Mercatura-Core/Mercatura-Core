...
class AbandonConflictTest(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 2
        self.extra_args = [["-minrelaytxfee=0.01"], []]
    ...
    def run_test(self):
        ...
        assert balance - newbalance < Decimal("1.00")  #no more than fees lost
        balance = newbalance
        ...
        outputs[alice.getnewaddress()] = Decimal("14.88")
        ...
        nAB = next(tx_out["vout"] for tx_out in alice.gettransaction(txAB1)["details"] if tx_out["amount"] == Decimal("14.88"))
        ...
        self.restart_node(0, extra_args=["-minrelaytxfee=0.02"])
        alice = self.nodes[0].get_wallet_rpc(self.default_wallet_name)
        ...
        newbalance = alice.getbalance()
        assert_equal(newbalance, balance - Decimal("20") + Decimal("14.88"))
        balance = newbalance

        # Send child tx again so it is unabandoned
        self.nodes[0].sendrawtransaction(signed2["hex"])
        newbalance = alice.getbalance()
        assert_equal(newbalance, balance - Decimal("10") - Decimal("14.88") + Decimal("24.76"))
        balance = newbalance
        ...
        assert_equal(len(self.nodes[0].getrawmempool()), 0)
        newbalance = alice.getbalance()
        assert_equal(newbalance, balance - Decimal("24.76"))
        balance = newbalance
        ...
        inputs.append({"txid": txA, "vout": nA})
        outputs = {}
        outputs[self.nodes[1].getnewaddress()] = Decimal("3.97")
        outputs[bob.getnewaddress()] = Decimal("5.97")
